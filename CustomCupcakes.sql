...
CREATE TABLE IF NOT EXISTS CustomCupcakes.Orders (
    ...
    CONSTRAINT customersF2_ID FOREIGN KEY (customer_ID)
...
CREATE TABLE IF NOT EXISTS CustomCupcakes.Cupcakes (
    ...
    CONSTRAINT icingF_ID FOREIGN KEY (cupcakeIcing_ID)
        REFERENCES CustomCupcakes.CupcakeIcing (cupcakeIcing_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT flavorF_ID FOREIGN KEY (cupcakeFlavor_ID)
        REFERENCES CustomCupcakes.CupcakeFlavor (cupcakeFlavor_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT fillingF_ID FOREIGN KEY (cupcakeFilling_ID)
        REFERENCES CustomCupcakes.CupcakeFilling (cupcakeFilling_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT orderF_ID FOREIGN KEY (order_ID)
...
CREATE TABLE IF NOT EXISTS CustomCupcakes.Favorites (
    ...
    CONSTRAINT customerF_ID FOREIGN KEY (customer_ID)
        REFERENCES CustomCupcakes.Customers (customer_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT cupcakesF_ID FOREIGN KEY (cupcake_ID)
        REFERENCES CustomCupcakes.Cupcakes (cupcake_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION
)  ENGINE=InnoDB COMMENT='A listing of a Customers Favorite Cupcakes and their compone' /* comment truncated */;
...
CREATE TABLE IF NOT EXISTS CustomCupcakes.CupcakeToppings (
    ...
    CONSTRAINT cupcakeF_ID FOREIGN KEY (cupcake_ID)
        REFERENCES CustomCupcakes.Cupcakes (cupcake_ID)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT toppingF_ID FOREIGN KEY (topping_ID)
